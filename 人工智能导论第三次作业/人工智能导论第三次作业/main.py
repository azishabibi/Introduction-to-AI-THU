...
def update_params_for_one_item(K, n_word_types, word_indices, counts, gammas, phi_d, log_betas, alpha, max_iter_d,
    ...
    while i < max_iter_d and delta >= tol_d:
        ...
        while n < n_word_types:
            ...
            ## Your codes here.
            for j in range(K):
                phi_d[n,j] = np.exp(log_betas[w,j]+psi_gammas[j])
            phi_d[n,:] /= np.sum(phi_d[n,:])
            gammas = alpha+c*phi_d[n,:]
            n += 1
        psi_gammas = psi(gammas)
...
def compute_bound_for_one_item(K, n_word_types, word_indices, count_vector, alpha, gammas, psi_gammas, phi_d,
    ...
    # psi_sum_gammas = psi(sum_gammas)
    # sum_psi_gammas = np.sum(psi(gammas))
    # sum_gammaln_gammas = np.sum(gammaln(gammas))
    bound += gammaln(K*alpha)-K*gammaln(alpha)+(alpha-1)*(np.sum(psi(gammas))-K*psi(sum_gammas))-gammaln(sum_gammas)+np.sum(gammaln(gammas))
    for i in range(K):
        bound -= (gammas[i]-1)*(psi_gammas[i]-psi(sum_gammas))
    for n in range(n_word_types):
        c = count_vector[n]
        for i in range(K):
            bound += c*phi_d[n,i]*(psi_gammas[i]-psi(sum_gammas))
            bound += c*phi_d[n,i]*log_betas[word_indices[n],i]
            bound -= c*phi_d[n,i]*np.log(phi_d[n,i])
    return bound
