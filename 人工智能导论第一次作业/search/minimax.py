    ...
    def get_action(self, state: State):
        ...
        def minimax_search(s: State) -> Tuple:
            ...
            if end:
                ...
                # TODO
                value = -inf if s.get_current_player() == self.player else inf
                action = None
                for a in s.get_all_actions():
                    s_copy = deepcopy(s)
                    s_copy.perform_action(a)
                    succ_v, _ = minimax_search(s_copy)
        ...
        def alpha_beta_search(s: State, alpha, beta):
            ...
            if end:
                ...
                for a in s.get_all_actions():
                    s_copy = deepcopy(s)
                    s_copy.perform_action(a)
                    succ_v, _ = alpha_beta_search(s_copy, alpha, beta)
                    if s.get_current_player() == self.player:
                        if succ_v > value:
                            value = succ_v
                            action = a
    ...
    def get_action(self, state: State):
        ...
        def cutting_off_alpha_beta_search(s: State, d, alpha, beta):
            ...
            if end:
                ...
                #action = None
                if d == 0:
                    value = CuttingOffAlphaBetaSearchPlayer.evaluation(self, s)
                    return value,action
                for a in s.get_all_actions():
                    s_copy = deepcopy(s)
                    s_copy.perform_action(a)
                    ...
                    if s.get_current_player() == self.player:
                        succ_v, _ = cutting_off_alpha_beta_search(s_copy,d, alpha, beta)
                        if succ_v > value:
                            value = succ_v
                            action = a
                        if value >= beta:
                        ...
                        succ_v, _ = cutting_off_alpha_beta_search(s_copy,d-1, alpha, beta)
